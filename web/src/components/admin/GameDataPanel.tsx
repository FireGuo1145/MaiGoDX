...
const blankEvent = (): Omit<GameEvent, "ID"> => ({
  type: 0,
  startDate: "",
  endDate: "",
  disableArea: "",
})
const blankCharge = (): GameCharge => ({
  chargeId: 0,
  orderId: 0,
  price: 0,
  startDate: "",
...
function EventEditor({
  ...
  event: GameEvent
  onSaved: (event: GameEvent) => Promise<void>
  onDeleted: (event: GameEvent) => Promise<void>
}) {
    ...
    <Card className="border-border bg-card">
      <CardContent className="space-y-3 p-4">
        <div className="flex items-center justify-between gap-3">
          <Badge className="border-neutral-500/25 bg-neutral-500/15 text-neutral-200">
            事件 #{event.ID}
          </Badge>
          <Button
            type="button"
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Input
            ...
            value={draft.type}
            onChange={(item) =>
              setDraft({ ...draft, type: Number(item.target.value) })
            }
            placeholder="事件类型"
          ...
          />
          <Input
            value={draft.disableArea}
            onChange={(item) =>
              setDraft({ ...draft, disableArea: item.target.value })
            }
            placeholder="禁用区域（可选）"
            className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
          />
          <Input
            ...
            onChange={(item) =>
              setDraft({ ...draft, startDate: item.target.value })
            }
            placeholder="开始时间"
            className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
          />
          <Input
            value={draft.endDate}
            onChange={(item) =>
              setDraft({ ...draft, endDate: item.target.value })
            }
            placeholder="结束时间"
            className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
          />
...
function ChargeEditor({
  ...
  charge: GameCharge
  onSaved: (charge: GameCharge) => Promise<void>
  onDeleted: (charge: GameCharge) => Promise<void>
}) {
      ...
      <CardContent className="space-y-3 p-4">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Input
            ...
            value={draft.orderId}
            onChange={(item) =>
              setDraft({ ...draft, orderId: Number(item.target.value) })
            }
            placeholder="排序 ID"
            className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
          />
          <Input
            ...
            value={draft.price}
            onChange={(item) =>
              setDraft({ ...draft, price: Number(item.target.value) })
            }
            placeholder="价格"
            className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
          />
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-foreground">
              <CalendarDays size={18} /> 游戏事件
            </CardTitle>
            <CardDescription className="text-muted-foreground">
              配置 GetGameEventApi 下发的事件记录。
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            >
              ...
              />
              <Input
                value={eventDraft.disableArea}
                onChange={(item) =>
                  setEventDraft({
                    ...eventDraft,
                    disableArea: item.target.value,
                  })
                }
                placeholder="禁用区域（可选）"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              ...
              <Input
                ...
                onChange={(item) =>
                  setEventDraft({ ...eventDraft, startDate: item.target.value })
                }
                placeholder="开始时间"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              />
              <Input
                ...
                onChange={(item) =>
                  setEventDraft({ ...eventDraft, endDate: item.target.value })
                }
                placeholder="结束时间"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              />
              <Button
                ...
                className="bg-neutral-600 hover:bg-neutral-500 sm:col-span-2"
              >
                <Plus size={16} />{" "}
                {creatingEvent ? "正在创建…" : "新增游戏事件"}
              </Button>
            </form>
            <div className="space-y-3">
              {events.length ? (
                ...
                <p className="text-sm text-muted-foreground">
                  暂无游戏事件。创建后将由 GetGameEventApi 下发。
                </p>
              )}
            </div>
          </CardContent>
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-foreground">
              <CircleDollarSign size={18} /> 收费项目
            </CardTitle>
            <CardDescription className="text-muted-foreground">
              配置 GetGameChargeApi 下发的收费项目。
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <form
            ...
            >
              <Input
                ...
                min="1"
                type="number"
                value={chargeDraft.chargeId || ""}
                onChange={(item) =>
                  setChargeDraft({
                    ...chargeDraft,
                    chargeId: Number(item.target.value),
                  })
                }
                placeholder="收费 ID"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              ...
              <Input
                required
                type="number"
                value={chargeDraft.orderId}
                onChange={(item) =>
                  setChargeDraft({
                    ...chargeDraft,
                    orderId: Number(item.target.value),
                  })
                }
                placeholder="排序 ID"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              ...
              <Input
                ...
                min="0"
                type="number"
                value={chargeDraft.price}
                onChange={(item) =>
                  setChargeDraft({
                    ...chargeDraft,
                    price: Number(item.target.value),
                  })
                }
                placeholder="价格"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              ...
              />
              <Input
                value={chargeDraft.endDate}
                onChange={(item) =>
                  setChargeDraft({ ...chargeDraft, endDate: item.target.value })
                }
                placeholder="结束时间"
                className="h-10 rounded-md border border-border bg-muted px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
              />
              <Button
                type="submit"
                isDisabled={creatingCharge}
                className="bg-neutral-600 hover:bg-neutral-500 sm:col-span-2"
              >
                <Plus size={16} />{" "}
                {creatingCharge ? "正在创建…" : "新增收费项目"}
              </Button>
            </form>
            <div className="space-y-3">
              {charges.length ? (
                charges.map((charge) => (
                  <ChargeEditor
                    ...
                    charge={charge}
                    onSaved={saveCharge}
                    onDeleted={deleteCharge}
                  />
                ))
              ) : (
                <p className="text-sm text-muted-foreground">
                  暂无收费项目。创建后将由 GetGameChargeApi 下发。
                </p>
              )}
            </div>
          </CardContent>
